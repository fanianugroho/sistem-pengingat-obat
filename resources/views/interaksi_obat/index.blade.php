...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title"> Interaksi Obat
                        <button type="button" class="btn btn-primary btn-rounded float-right mb-3"
                            @click="createModal()"><i class="fas fa-plus-circle"></i> Tambah Interaksi</button>
                    </h4>
                        ...
                        <table id="table" class="table table-striped table-bordered no-wrap">
                            ...
                            <tbody>
                                <tr v-for="item, index in mainData" :key="index">
                                    <td>@{{ index+1 }}</td>
                                    <td>@{{ item.nama_interaksi == 'null' ? '' : item.nama_interaksi}}</td>
                                    <td>
    ...
    <div class="modal-dialog modal-lg" id="modal">
        <div class="modal-content">
            <div class="modal-header ">
                <h4 class="modal-title" v-show="!editMode" id="myLargeModalLabel">Tambah Interaksi</h4>
...
<script>
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            storeData() {
                this.form.post("{{ route('interaksiobat.store') }}")
                    .then(response => {
                        $('#modal').modal('hide');
                        Swal.fire(
                            'Berhasil',
                            'Interaksi Obat berhasil ditambahkan',
                            'success'
                        )
                        this.refreshData()
                    })
            ...
            updateData() {
                url = "{{ route('interaksiobat.update', ':id') }}".replace(':id', this.form.id)
                this.form.put(url)
                    .then(response => {
                        $('#modal').modal('hide');
                        Swal.fire(
                            'Berhasil',
                            'Interaksi Obat berhasil diubah',
                            'success'
                        )
                        this.refreshData()
                    })
            ...
            deleteData(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        url = "{{ route('interaksiobat.destroy', ':id') }}".replace(':id', id)
                        this.form.delete(url)
                            .then(response => {
                                Swal.fire(
                                    'Terhapus',
                                    'Interaksi Obat telah dihapus',
                                    'success'
                                )
                                this.refreshData()
                            })
